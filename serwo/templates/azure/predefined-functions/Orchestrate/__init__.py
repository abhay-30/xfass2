...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    tuxn = yield context.call_activity("graphGen", serwoObject)
    csbe = []
    abiu = context.call_activity("graphBft", tuxn)
    npbr = context.call_activity("graphMst", tuxn)
    gyzq = context.call_activity("pagerank", tuxn)
    csbe.append(abiu)
    csbe.append(npbr)
    csbe.append(gyzq)
    woya = yield context.task_all(csbe)
    cwvm = yield context.call_activity("aggregate", woya)
    cwvm = insert_end_stats_in_metadata(cwvm)
    nsor = yield context.call_activity("CollectLogs", cwvm)
    return nsor
